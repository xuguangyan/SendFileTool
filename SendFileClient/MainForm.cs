...
namespace SendFileClient
{
    ...
    {
        private NetClient sendUdpClient;
        ...
        //ini配置结点名
        private string iniSession = "DS_SendFileClient";
        ...
        private void btnconnect_Click(object sender, EventArgs e)
        {
            if (!bConnect)
            {
                ...
                if (rdBtnTCP.Checked)
                {
                    sendUdpClient = new NetClient(localIpEndPoint, "TCP");
                }
                else
                {
                    sendUdpClient = new NetClient(localIpEndPoint);
                }

                IPAddress remoteIp = IPAddress.Parse(txtIP.Text);
                IPEndPoint remoteIpEndPoint = new IPEndPoint(remoteIp, int.Parse(txtPort.Text));
                ...
                //发送登录连接信息
                MsgEntity msg = new MsgEntity(Constant.MSG_TYPE_CMD, Constant.SESSION_CODE_LOGIN);
            ...
            while (true)
            {
                try
                {
                    ...
                    else if (msg.msgType == Constant.MSG_TYPE_CMD)//收到命令消息
                    {
                        ...
                        {
                            InvokeCtrl.UpdateCtrlText(btnconnect, "连接");
                            bConnect = false;
        ...
        //自动保存定时器
        private void timerSaveCfg_Elapsed(object sender, ElapsedEventArgs e)
        {
            //写入配置文件
            WriteConfig();
        }
        ...
        //传输状态定时器
        private void timerSendState_Elapsed(object sender, ElapsedEventArgs e)
        {
            //传输速度更新
            if (counter == 0)
            {
        ...
        private void MainForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (e.CloseReason == CloseReason.UserClosing)
            {
                ...
                this.Hide();
                menuItemShow.Text = "显示界面(&V)";
                this.notifyIcon.ShowBalloonTip(1000, "文件发送-客户端", "程序已最小化到这里，双击可以打开", ToolTipIcon.Info);
                return;
            }
        }
